...
class VARTrainer(object):
    ...
    @torch.no_grad()
    def eval_ep_per_scale(self, ld_val: DataLoader):
        tot = 0
        ...
        for scale_idx in range(self.num_scales):
            for inp_B3HW, label_B in ld_val:
                ...
                logits_BLV_per_scale = self.var_wo_ddp(label_B, x_BLCv_per_scale)
                L_mean += self.val_loss(logits_BLV_per_scale.data.view(-1, V), gt_BL_per_scale.view(-1)) * B
                acc_mean += (logits_BLV_per_scale.data.argmax(dim=-1) == gt_BL_per_scale).sum() * (100/gt_BL_per_scale.shape[1])
                tot += B
    ...
    def train_step_per_scale(
        self, it: int, g_it: int, stepping: bool, metric_lg: MetricLogger, tb_lg: TensorboardLogger,
        inp_B3HW: FTen, label_B: Union[ITen, FTen], prog_si: int, prog_wp_it: float, scale_idx: int, 
        ...
        if g_it == 0 or (g_it + 1) % 500 == 0:
            prob_per_class_is_chosen = pred_BL_per_scale.view(-1).bincount(minlength=V).float()
