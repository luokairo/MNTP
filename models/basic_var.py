...
__all__ = ['FFN', 'AdaLNSelfAttn', 'AdaLNBeforeHead', 'AdaLNSelfAttnC2I']
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x_q, x_kv):
        # x_q: B L C，x_kv: B Lc C
        B, L, C = x_q.shape
        q = self.q_proj(x_q).view(B, L, self.num_heads, self.head_dim)
        kv = self.kv_proj(x_kv).view(B, -1, 2, self.num_heads, self.head_dim)
        k, v = kv.unbind(dim=2)  # BLcHc
        ...
        if using_flash:
            # flash_attn 期望 BLHc
            oup = flash_attn_func(q.to(main_type), k.to(main_type), v.to(main_type),
                                  dropout_p=self.attn_drop if self.training else 0.0,
                                  softmax_scale=self.scale).view(B, L, C)
        elif self.using_xform:
            oup = memory_efficient_attention(
                q.to(main_type), k.to(main_type), v.to(main_type),
                attn_bias=None, p=self.attn_drop if self.training else 0.0, scale=self.scale
            ).view(B, L, C)
        else:
            q = q.permute(0, 2, 1, 3)  # BHLc
            k = k.permute(0, 2, 1, 3)
            v = v.permute(0, 2, 1, 3)
            oup = slow_attn(q, k, v, scale=self.scale, attn_mask=None,
                            dropout_p=self.attn_drop if self.training else 0.0).transpose(1, 2).reshape(B, L, C)
...
class AdaLNPerScale(nn.Module):
    def __init__(self, hidden_dim: int, cond_dim: int, num_scales: int = 10):
        ...
        self.scale_embed = nn.Embedding(num_scales, cond_dim)

    def forward(self, x: torch.Tensor, cond: torch.Tensor, scale_idx: int):
        if not isinstance(scale_idx, torch.Tensor):
            scale_idx = torch.tensor(scale_idx, dtype=torch.long, device=cond.device)
        scale_idx = scale_idx.to(device=cond.device, dtype=torch.long)

        cond_scaled = cond + self.scale_embed(scale_idx)
        gamma, beta = self.cond_proj(cond_scaled).chunk(2, dim=-1)  # (B,C),(B,C)
...
class AdaLNSelfAttn(nn.Module):
    def __init__(
        ...
        self.cross = CrossAttention(embed_dim=embed_dim, num_heads=num_heads, attn_drop=attn_drop, proj_drop=drop, attn_l2_norm=attn_l2_norm, flash_if_available=flash_if_available)
        ...
        if self.shared_aln:
            ...
            self.ada_gss_cross = nn.Parameter(torch.randn(1, 1, 3, embed_dim) / embed_dim**0.5)
        else:
            lin = nn.Linear(cond_dim, 6*embed_dim)
            self.ada_lin = nn.Sequential(nn.SiLU(inplace=False), lin)
            self.ada_lin_cross= nn.Sequential(nn.SiLU(inplace=False), nn.Linear(cond_dim, 3*embed_dim))
        
        self.fused_add_norm_fn = None
    
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, cond_BD, attn_bias, cond_tokens=None):   # C: embed_dim, D: cond_dim
        if self.shared_aln:
        ...
        if cond_tokens is not None:
            if self.shared_aln:
                scale_c, shift_c, gamma_c = (self.ada_gss_cross + cond_BD).unbind(2)  # 3×B1C
            else:
                scale_c, shift_c, gamma_c = self.ada_lin_cross(cond_BD).view(-1,1,3,self.C).unbind(2)

            x = x + self.drop_path(
                self.cross(self.ln_wo_grad(x).mul(scale_c.add(1)).add_(shift_c), cond_tokens).mul_(gamma_c)
            )
...
# ---------------- 类条件版本（C2I） ----------------
class AdaLNSelfAttnC2I(nn.Module):
    def __init__(
        self, block_idx, last_drop_p, embed_dim, cond_dim, shared_aln: bool, norm_layer,
        num_heads, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0., attn_l2_norm=False,
        flash_if_available=False, fused_if_available=True,
    ...
    def forward(self, x, cond_BD, attn_bias, scale_idx: int):
        # Self-Attention
        x = x + self.drop_path(self.attn(self.adaln1(x, cond_BD, scale_idx), attn_bias))
        # FFN
        x = x + self.drop_path(self.ffn(self.adaln2(x, cond_BD, scale_idx)))
        return x


# ---------------- 文本条件版本（T2I） ----------------
class AdaLNSelfAttnT2I(nn.Module):
    def __init__(
        self, block_idx, last_drop_p, embed_dim, cond_dim, shared_aln: bool, norm_layer,
        ...
        self.attn = SelfAttention(block_idx=block_idx, embed_dim=embed_dim, num_heads=num_heads,
                                  attn_drop=attn_drop, proj_drop=drop, attn_l2_norm=attn_l2_norm,
                                  flash_if_available=flash_if_available)
        self.cross = CrossAttention(embed_dim=embed_dim, num_heads=num_heads,
        ...
        if self.shared_aln:
            ...
            self.ada_lin       = nn.Sequential(nn.SiLU(inplace=False), nn.Linear(cond_dim, 6*embed_dim))
            self.ada_lin_cross = nn.Sequential(nn.SiLU(inplace=False), nn.Linear(cond_dim, 3*embed_dim))

    def forward(self, x, cond_BD, attn_bias, cond_tokens):
        # γ/β/scale/shift from cond_BD
        if self.shared_aln:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(2)
            scale_c, shift_c, gamma_c = (self.ada_gss_cross + cond_BD).unbind(2)
        else:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = \
                self.ada_lin(cond_BD).view(-1,1,6,self.C).unbind(2)
            scale_c, shift_c, gamma_c = \
                self.ada_lin_cross(cond_BD).view(-1,1,3,self.C).unbind(2)

        # Self-Attention
        x = x + self.drop_path(
            self.attn(self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1),
                      attn_bias=attn_bias).mul_(gamma1)
        )
        # Cross-Attention
        x = x + self.drop_path(
            self.cross(self.ln_wo_grad(x).mul(scale_c.add(1)).add_(shift_c),
                       cond_tokens).mul_(gamma_c)
        )
        # FFN
        x = x + self.drop_path(
