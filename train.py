...
from utils.data_sampler import DistInfiniteBatchSampler, EvalDistributedSampler, DynamicDistBatchSampler
from utils.misc import auto_resume

from torch.utils.data._utils.collate import default_collate

def make_collate_with_scale(sampler):
    def collate(batch):  # batch 是 [(inp, label), (inp, label), ...]
        data = default_collate(batch)  # -> (inp_B3HW, label_B)
        # 把当前步的随机尺度附带上
        scale_idx = sampler.current_scale
        return data[0], data[1], scale_idx
    return collate
...
def build_everything(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        # Build dataloader: dynamic sample
        scale2ratio = {0: 1, 1: 0.5, 2: 0.33, 3: 0.25, 4: 0.2, 5: 0.167, 6: 0.125, 7: 0.1, 8: 0.077, 9: 0.0625}
        scale2bs = {k: v * args.glb_batch_size for k, v in scale2ratio.items()}

        sampler = DynamicDistBatchSampler(
            scale2bs=scale2bs,
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            val_loss_mean, val_loss_tail, val_acc_mean, val_acc_tail, tot, cost = trainer.eval_ep_per_scale(ld_val)
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, label, scale_idx) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        if it == start_it:
            accum_samples = 0
            base_bs = args.batch_size  # 参考 batchsize（比如最大尺度）
            accum_target = base_bs * args.ac  # 理想累积样本数

        # 当前 batch size（每个 GPU）
        cur_bs = inp.size(0)

        # 累加样本数
        accum_samples += cur_bs
        stepping = accum_samples >= accum_target
        if stepping:
            accum_samples = 0
        step_cnt += int(stepping)
        
        grad_norm, scale_log2 = trainer.train_step_per_scale(
            it=it, g_it=g_it, stepping=stepping, metric_lg=me, tb_lg=tb_lg,
            inp_B3HW=inp, label_B=label, prog_si=prog_si, prog_wp_it=args.pgwp * iters_train, scale_idx=scale_idx
        )
