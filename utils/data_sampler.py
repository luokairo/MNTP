...
class DynamicDistBatchSampler(DistInfiniteBatchSampler):
    def __init__(self, scale2bs, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.scale2bs = scale2bs
        self.num_scales = len(scale2bs)
    ...
    def __iter__(self):
        ...
        while True:
            ...
            while p < self.max_p:
                # 1) 随机挑一个尺度
                self.current_scale = int(torch.randint(0, self.num_scales, (1,)).item())
                # 2) 本步的 global/local batch size
                glb_bs = int(self.scale2bs[self.current_scale])
                local_bs = glb_bs // self.world_size
                # 3) 从本 rank 的 local 索引池里随机抽 local_bs 个样本
                if local_bs <= 0:
                    local_bs = 1
                if len(self.indices) < local_bs:
                    # 兜底：扩充或重排一次
                    self.indices = self.gener_indices()
                start = torch.randint(0, max(1, len(self.indices) - local_bs + 1), (1,)).item()
                batch_indices = self.indices[start:start + local_bs]
                p += local_bs
                # 只返回 indices（DataLoader 规范）
                yield batch_indices
            if self.shuffle:
                self.indices = self.gener_indices()
